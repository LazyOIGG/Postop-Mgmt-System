...
def get_data(path,max_len=None):
 ...
 # 使用Aho-Corasick算法快速匹配词典中的实体
    # 作用: 提高实体召回率，补充模型遗漏
    #rule_find 模块是实体识别系统的词典匹配专家，它的作用是确保已知实体100%被识别。 
class rule_find:
    def __init__(self):
...
class Bert_Model(nn.Module):
    def __init__(self,model_name,hidden_size,tag_num,bi):
        ...
        try:
            print(f"尝试加载BERT模型: {model_name}")
            self.bert = BertModel.from_pretrained(model_name, local_files_only=True)
            print("成功使用local_files_only=True加载模型")
        except Exception as e1:
            print(f"使用local_files_only=True加载失败: {str(e1)}")
            try:
